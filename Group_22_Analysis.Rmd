...
Section \ref{sec:eda} consists of an exploratory data analysis of the data. Section \ref{sec:fda} contains the contains the formal analysis of fitting the GLMs, the model selection and data visualizations resulting from the model selected. Section \ref{sec:con} contains our conclusions. Before the Exploratory Data Analysis part we first summary the NA. From the table \ref{tab:summaryna} we can see that our "depth" data have 213 missing data which is almost 43 percentage of the dataset so we first delete the row which both depth, height and width are NA and then replace the missing data use the MI method.
...
#summ(model_full)
#AIC= 292.59, BIC= 382.65, least significant term Sellable_online with p-value 0.99
...
#summ(model2)
#AIC= 291.46, BIC= 377.41, least significant term category with p-values 0.99
...
#summ(model3)
#AIC 306.88, BIC 327.35, least significant term other_colors with p-value 0.07
...
summ(model4)
#AIC 308.37, BIC 324.74, all terms significant

#trying models with an interaction
mod5 <- glm2(Expensive~ height*depth + width,data = data4, family = binomial(link = "logit"))
#summ(mod5)
#AIC 305.96, BIC 326.43
mod6 <- glm2(Expensive~ depth*width + height,data = data4, family = binomial(link = "logit"))
#summ(mod6)
#AIC 308.77, BIC 329.24
mod7 <- glm2(Expensive~ width*height + depth,data = data4, family = binomial(link = "logit"))
#summ(mod7)
#AIC 309.88, BIC 330.34
#none of these models are an improvement so we use model4
```

We see that the intercept is negative, the coefficients for height, width and depth are positive. The coefficient for depth is larger than height and width. This suggests depth is more likely to influence the price while height and width are less likely to influence the price.

The best fitted model as follows
$$
...
Hence, the log-odds of the price being expensive increase by $\beta_1$ for every one unit increase in height, $\beta_2$ for width, $\beta_3$ for depth. And the depth is the most important variable for price over 1000 Saudi Riyals.
...
```
From the table \ref{tab:con} we can see that all the interval does not contain zero so all variables are significant.
...
Hence the point estimate for the log-odds is showing below, Figure \ref{fig:logodds}.

```{r, echo=FALSE, warning=FALSE, message=FALSE,eval = TRUE,out.width="70%",fig.align = "center",fig.pos = "H",fig.cap="\\label{fig:logodds} Log-Odds in expensive",fig.show='asis'}
#plotting log odds
...
$$
The point estimate for the odds is shows below, Figure \ref{fig:odds}.
...
```
Create the probability of price being expensive plot, the plot is as follows for each variable.

```{r, echo=FALSE, warning=FALSE, message=FALSE, eval = TRUE,out.width="70%",fig.align = "center",fig.pos = "H",fig.cap="\\label{fig:probplot} Probability plot",fig.show='asis'}
...
From the model we can see that, the furniture's category, whether they sell online and whether it have different colors may not have significant influence on the price over 1000 Saudi Riyals. While the height, depth and width of the furniture have significant influence on the price over 1000 Saudi Riyals and the furniture's depth have the biggest influence. And as the furniture become larger (the value height, depth and width become bigger) the probability of its price will be more expensive, which means its probability that the price over 1000 Saudi Riyals will close to one.
